...
void expectNoIdentityFields(WidgetTester tester) {
  ...
  for (final field in fields) {
    ...
    final textField =
        tester.widget<TextField>(find.descendant(
            of: find.byWidget(field), matching: find.byType(TextField)));
    ...
    for (final pattern in _identityPatterns) {
      expect(pattern.hasMatch(haystack), isFalse,
          reason: 'form field must not reference an identity document: '
  ...
  for (final text in tester.widgetList<Text>(find.byType(Text))) {
    for (final pattern in _identityPatterns) {
      expect(pattern.hasMatch(text.data ?? ''), isFalse,
          reason: 'screen text must not reference an identity document: '
...
void expectNoIdentityPayloadKeys(Map<String, dynamic> payload) {
  for (final key in payload.keys) {
    for (final pattern in _identityPatterns) {
      expect(pattern.hasMatch(key), isFalse,
          reason: 'saveProfile payload key "$key" must not reference an '
...
void main() {
  group('OP-R2 — OnboardingScreen form (part 1, task 2.5)', () {
    testWidgets('form contains no cédula/documento field', (tester) async {
      ...
      expect(find.widgetWithText(TextFormField, 'Nombre completo'),
          findsOneWidget);
      expect(find.widgetWithText(TextFormField, 'Tu moto (marca y modelo)'),
          findsOneWidget);
      expect(find.widgetWithText(TextFormField, 'Ciudad'), findsOneWidget);
    });

    testWidgets('saveProfile payload has no identity-document key', (tester) async {
      final repo = MockProfileRepository();
      await tester.pumpWidget(MaterialApp(
        home: OnboardingScreen(
          repository: repo,
      ...
      expect(repo.saveCalls.first.keys,
  ...
  group('OP-R2 — ProfileEditScreen form (part 2, task 3.5)', () {
    testWidgets('edit form contains no cédula/documento field', (tester) async {
      final repo = MockProfileRepository(row: {
        'full_name': 'Ana María',
