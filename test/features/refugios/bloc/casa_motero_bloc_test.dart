...
void main() {
  group('MotoposadasBloc — casa_motero (F-M9/F-M11)', () {
    ...
    test('CreateCasaMotero event holds all fields incl. exact coords + disclaimer', () {
    ...
    test('CreateCasaMotero event equality works', () {
      const a = CreateCasaMotero(
        title: 'A', description: '', maxGuests: 1,
        lat: 1, lng: 2, latExact: 3, lngExact: 4,
        whatsappPhone: '300', disclaimerAcceptedAt: null,
      );
      ...
      const c = CreateCasaMotero(
        title: 'B', description: '', maxGuests: 1,
        lat: 1, lng: 2, latExact: 3, lngExact: 4,
        whatsappPhone: '300', disclaimerAcceptedAt: null,
      );
      expect(a, equals(b));
    ...
    test('eligibility selects id ONLY — never private columns (M-MAPA-1)', () async {
      final client = FakeSupabaseClient(user: _user());
      client.tables['motoposadas'] =
          FakeQueryBuilder(result: null, recorder: client.calls);
    ...
    test('eligibility with existing row → has=true (max-1 UX gate)', () async {
      final client = FakeSupabaseClient(user: _user());
      client.tables['motoposadas'] =
          FakeQueryBuilder(result: {'id': 7}, recorder: client.calls);
      final bloc = MotoposadasBloc(client: client);
    ...
    test('create invokes create_casa_motero RPC with normalized phone + exact params', () async {
      final client = FakeSupabaseClient(user: _user());
      ...
      for (final key in params.keys) {
        expect(key.contains('cedula'), isFalse, reason: 'no identity key: $key');
        expect(key.contains('documento'), isFalse, reason: 'no identity key: $key');
      }
      expect(params.containsKey('p_user_id'), isFalse);
      await bloc.close();
    });

    test('duplicate create (23505) → CasaMoteroAlreadyExists, never crash (M-CRUD-1)', () async {
      final client = FakeSupabaseClient(user: _user());
      ...
      final state = await _lastStateAfter(
        bloc,
        CreateCasaMotero(
          title: 'Segunda casa', description: '', maxGuests: 2,
          lat: 4.6, lng: -74.0, latExact: 4.5, lngExact: -74.1,
    ...
    test('other RPC error → MotoposadasError with message (no crash)', () async {
      final client = FakeSupabaseClient(user: _user());
      client.rpcErrors['create_casa_motero'] = const PostgrestException(
      ...
      final state = await _lastStateAfter(
        bloc,
        CreateCasaMotero(
          title: 'X', description: '', maxGuests: 1,
          lat: 4.6, lng: -74.0, latExact: 4.5999, lngExact: -74.0001,
          whatsappPhone: '3001234567',
          disclaimerAcceptedAt: DateTime.utc(2026, 8, 5),
        ),
      );
    ...
    test('UpdateCasaMotero → mp_update_own with approx coords + is_active toggle', () async {
      final client = FakeSupabaseClient(user: _user());
      client.tables['motoposadas'] =
          FakeQueryBuilder(result: null, recorder: client.calls);
      final bloc = MotoposadasBloc(client: client);
    ...
    test('UpdateCasaMoteroDetails → cmd_update_own with normalized phone', () async {
      final client = FakeSupabaseClient(user: _user());
      client.tables['casa_motero_details'] =
          FakeQueryBuilder(result: null, recorder: client.calls);
      final bloc = MotoposadasBloc(client: client);
    ...
    test('FetchCasaMoteroWhatsapp invokes RPC with p_id and emits phone', () async {
      final client = FakeSupabaseClient(user: _user());
      ...
      final rpcCalls = _callsWhere(client, #rpc);
      expect(rpcCalls, hasLength(1));
      expect(rpcCalls.first.positionalArguments.first, 'get_motoposada_whatsapp');
      expect(_rpcParams(rpcCalls.first), {'p_id': 42});
      await bloc.close();
    });

    test('FetchCasaMoteroWhatsapp NULL phone → phone null (no disponible)', () async {
      final client = FakeSupabaseClient(user: _user());
      client.rpcResults['get_motoposada_whatsapp'] = null; // inactive/other type
      final bloc = MotoposadasBloc(client: client);
    ...
    test('LoadCasaMoteroDetails selects owner-only details for prefill', () async {
      final client = FakeSupabaseClient(user: _user());
      ...
      expect(fromCalls.first.positionalArguments.first, 'casa_motero_details');
      final eqCalls = _callsWhere(client, #eq).toList();
      expect(eqCalls[0].positionalArguments, ['user_id', 'user-1']);
    ...
    test('non-owner public update rejected by RLS → error, exactly one attempt', () async {
      final client = FakeSupabaseClient(user: _user());
      ...
      final state = await _lastStateAfter(
        bloc,
        const UpdateCasaMotero(
          id: 7, title: 'X', description: '', maxGuests: 1,
          lat: 4.6, lng: -74.0, isActive: true,
        ),
      );
    ...
    test('non-owner delete rejected by RLS → error, no partial write', () async {
      final client = FakeSupabaseClient(user: _user());
      client.tables['motoposadas'] = FakeQueryBuilder(
        error: const PostgrestException(
          message: 'new row violates row-level security policy',
      ...
      final state = await _lastStateAfter(bloc, const DeleteMotoposada(id: 7));
