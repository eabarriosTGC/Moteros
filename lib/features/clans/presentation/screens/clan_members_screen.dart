/// Clan Members Screen — AsfaltoClub Battle Ride.
...
class ClanMembersScreen extends StatefulWidget {
  final String clanId;
  const ClanMembersScreen({super.key, required this.clanId});
...
class _ClanMembersScreenState extends State<ClanMembersScreen> {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<ClanBloc, ClanState>(
      builder: (context, state) {
        ...
        if (state is ClanLoaded) return _buildScreen(state);
  ...
  Widget _buildScreen(ClanLoaded state) {
  ...
  Widget _buildInviteSection(ClanLoaded state) {
    return Container(
  ...
  Widget _buildMemberCard(ClanLoaded state, Map<String, dynamic> member, bool canManage) {
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text('Nivel ${member['level'] ?? 1} · $roleLabel',
  ...
  Widget _buildManageButton(ClanLoaded state, Map<String, dynamic> member) {
    final role = member['role'] as String? ?? 'rider';
    final memberId = member['user_id'] as String? ?? '';
      ...
      onSelected: (value) {
        switch (value) {
          ...
          case 'promote_rider':
            ...
            break;
          case 'demote_recruit':
            _changeRole(state, memberId, 'recruit');
            break;
          case 'kick':
  ...
  void _changeRole(ClanLoaded state, String memberId, String newRole) {
    HapticFeedback.lightImpact();
    context.read<ClanBloc>().add(UpdateMemberRole(
  ...
  void _confirmKick(ClanLoaded state, String memberId) {
    showDialog(
      context: context,
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              ...
              context.read<ClanBloc>().add(KickMember(
                clanId: widget.clanId,
                memberId: memberId,
              ));
            },
